...
public class DictamenLoteController {
	...
	public DictamenLoteResponseDto guardar(@Valid @RequestBody DictamenLoteRequestDto request) {
		DictamenLote dictamenLote = mapper.toDomain(request);
		return mapper.toResponseDto(dictamenLoteUseCase.guardar(dictamenLote));
	}
	...
	@PutMapping("/{idDictamen}")
	public ResponseEntity<DictamenLoteResponseDto> actualizar(
			@PathVariable int idDictamen,
			@Valid @RequestBody DictamenLoteRequestDto request) {
		DictamenLote dictamenLoteDatos = mapper.toDomain(request);
		DictamenLote dictamenLoteActualizado = dictamenLoteUseCase.actualizar(idDictamen, dictamenLoteDatos);
		return ResponseEntity.ok(mapper.toResponseDto(dictamenLoteActualizado));
	}
