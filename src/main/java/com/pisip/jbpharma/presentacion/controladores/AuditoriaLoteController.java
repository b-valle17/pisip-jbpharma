...
public class AuditoriaLoteController {
	...
	public AuditoriaLoteResponseDto guardar(@Valid @RequestBody AuditoriaLoteRequestDto request) {
		AuditoriaLote auditoriaLote = mapper.toDomain(request);
		return mapper.toResponseDto(auditoriaLoteUseCase.guardar(auditoriaLote));
	}
	...
	@PutMapping("/{idAuditoria}")
	public ResponseEntity<AuditoriaLoteResponseDto> actualizar(
			@PathVariable int idAuditoria,
			@Valid @RequestBody AuditoriaLoteRequestDto request) {
		AuditoriaLote auditoriaLoteDatos = mapper.toDomain(request);
		AuditoriaLote auditoriaLoteActualizado = auditoriaLoteUseCase.actualizar(idAuditoria, auditoriaLoteDatos);
		return ResponseEntity.ok(mapper.toResponseDto(auditoriaLoteActualizado));
	}
