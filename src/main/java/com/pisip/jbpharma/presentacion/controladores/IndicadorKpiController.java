...
public class IndicadorKpiController {
	...
	public IndicadorKpiResponseDto guardar(@Valid @RequestBody IndicadorKpiRequestDto request) {
		IndicadorKpi indicadorKpi = mapper.toDomain(request);
		return mapper.toResponseDto(indicadorKpiUseCase.guardar(indicadorKpi));
	}
	...
	@PutMapping("/{idKpi}")
	public ResponseEntity<IndicadorKpiResponseDto> actualizar(
			@PathVariable int idKpi,
			@Valid @RequestBody IndicadorKpiRequestDto request) {
		IndicadorKpi indicadorKpiDatos = mapper.toDomain(request);
		IndicadorKpi indicadorKpiActualizado = indicadorKpiUseCase.actualizar(idKpi, indicadorKpiDatos);
		return ResponseEntity.ok(mapper.toResponseDto(indicadorKpiActualizado));
	}
