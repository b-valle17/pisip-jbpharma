...
public class AlertaEnsayoUseCaseImpl implements iAlertaEnsayoUseCase {
	...
	@Override
	public AlertaEnsayo guardar(AlertaEnsayo nuevo) {
		nuevo.setIdAlerta(null);
		if (nuevo.getFechaGeneracion() == null)
			nuevo.setFechaGeneracion(LocalDateTime.now());
		return repositorio.guardar(nuevo);
	}

	@Override
	public AlertaEnsayo buscarPorId(long id) {
		return repositorio.buscarPorId(id).orElseThrow(() -> new RuntimeException("Alerta no encontrada"));
	}
	...
	@Override
	public AlertaEnsayo actualizar(long id, AlertaEnsayo datos) {
		AlertaEnsayo actual = buscarPorId(id);
		datos.setIdAlerta(id);
		if (datos.getFechaGeneracion() == null)
			datos.setFechaGeneracion(actual.getFechaGeneracion());
